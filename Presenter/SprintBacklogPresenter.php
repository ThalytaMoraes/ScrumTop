...
class CadastroSPBPresenter {
    ...
    public function __cadastroSimples($pestimativa, $status) {


        $MembroDAO = new SprintBacklogDAO();
        $MembroDAO->__initialize();
        try {
            $MembroDAO->__inserir($pestimativa, $status, $_SESSION['codigo']);
            $retorno = 1;
        } catch (Exception $e) {
            $retorno = 0;
